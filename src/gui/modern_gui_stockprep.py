...
class StockPrepApp:
    ...
    def load_image_preview(self, image_path: str):
        """Carga la vista previa de la imagen usando SafeImageManager - SOLUCIÓN COMPLETA"""
        try:
            ...
            if photo and image_key:
                ...
                logger.info(f"Vista previa cargada exitosamente: {Path(image_path).name}")
            else:
                # Error manejado automáticamente por SafeImageManager
                self.image_label.config(text="❌ Error cargando imagen", image="")
                logger.warning(f"No se pudo cargar la vista previa: {image_path}")
    ...
    def export_results(self):
        ...
        file_path = filedialog.asksaveasfilename(
            ...
            initialfile=f"stockprep_resultados_{datetime.now().strftime('%Y%m%d_%H%M%S')}{extension}"
        )
