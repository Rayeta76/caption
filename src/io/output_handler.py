...
class OutputHandler:
    def __init__(self, image_processor, status_callback: Callable = None):
        self.batch_engine = BatchEngine(image_processor, status_callback)

    def process_images(self, image_folder_path: str) -> List[dict]:
        """Procesa las imágenes en la carpeta especificada."""
        return self.batch_engine.run(image_folder_path)
